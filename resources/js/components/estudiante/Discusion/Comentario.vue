    ...
    <div id="comments-list" class="comments-list" v-if="comentario.id">
      <div>
        <div class="comment-main-level" v-if="comentario.parent == null">
          ...
          <div
            v-for="(comentarioch, indexch) in childrens"
            :key="indexch"
            class="childrens-c"
          >
            <Comentario
...
export default {
  ...
  data() {
    return {
      ...
      childrens: this.initialComentario.childrens,
      replicar: false,
    };
  },
  computed: {
  ...
  methods: {
    createcomentario(comentario) {
      if (comentario.id) {
        this.childrens.push(comentario);
        this.respuestas++;
    ...
    cerrarupdate(comentario) {
      comentario.childrens = this.initialComentario.childrens;
    ...
    eliminarc(value) {
      if (this.comentario.id == value) {
        ...
        axios
          ...
          .then((res) => {
            // const index = this.childrens.findIndex((item) => item.id === value);
